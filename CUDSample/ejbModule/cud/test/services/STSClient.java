...
public static Document issueToken(String type, String user_onbehalfof_token) throws Exception{
		...
		try {
			...
			System.out.println("dispatch:sign_validate:" +dispatch.getClass());
...
*/
			...
			NodeList signatureAssertionList_reply = soapBody_reply
					.getElementsByTagNameNS("*", "Signature");

			if (signatureAssertionList_reply == null
					|| signatureAssertionList_reply.getLength() == 0) {
				throw new Exception(
						"This Assertion requires <dsig:Signature>, which is missing!!!");
			}
			
			Node signatureAssertionNode1_reply = signatureAssertionList_reply.item(0);

			DOMValidateContext valContextAssertion1_reply = new DOMValidateContext(
					publicKey_reply, signatureAssertionNode1_reply);

			valContextAssertion1_reply.putNamespacePrefix(XMLSignature.XMLNS, "dsig");

			
			((Element)asserion).setIdAttribute("ID", true);
			
			javax.xml.crypto.dsig.XMLSignature signatureAssertion1_reply = fac
					.unmarshalXMLSignature(valContextAssertion1_reply);

			boolean resultAssertion1_reply = signatureAssertion1_reply
					.validate(valContextAssertion1_reply);

			System.out.println("dispatch:sign_Assertion_reply_validate:" + resultAssertion1_reply);
